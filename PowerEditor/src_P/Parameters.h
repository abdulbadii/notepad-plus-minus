...
struct LangMenuItem final
{
	...
	LangMenuItem(LangType lt, int cmdID = 0, const generic_string& langName = L""):
...
struct PrintSettings final {
	...
	bool isHeaderPresent() const {
		return ((_headerLeft != L"") || (_headerMiddle != L"") || (_headerRight != L""));
	};

	bool isFooterPresent() const {
		return ((_footerLeft != L"") || (_footerMiddle != L"") || (_footerRight != L""));
	};
...
struct NppGUI final
{
	...
	generic_string _commandLineInterpreter = L"cmd";
...
public:
	...
	{
		_themeList.push_back(std::pair<generic_string, generic_string>(L"Default (stylers.xml)", xmlFullPath));
	}
...
private:
	...
	int _nbRecentFile = 0;
	int _nbMaxRecentFile = NB_MAX_LRF_FILE;
...
private:
	...
	generic_string _appdataNppDir; // sentinel of the absence of "doLocalConf.xml" : (_appdataNppDir == L"")?"doLocalConf.xml present":"doLocalConf.xml absent"
