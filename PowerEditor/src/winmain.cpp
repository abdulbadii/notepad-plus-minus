...
void allowWmCopydataMessages(Notepad_plus_Window& notepad_plus_plus, const NppParameters& param, winVer ver){
	#ifndef MSGFLT_ADD
	...
	if (ver >= WV_VISTA || ver == WV_UNKNOWN)	{
		...
		if (hDll)	{
			...
			if (param.getWinVersion() == WV_VISTA)	{
...
ParamVector convertParamsToNotepadStyle(PWSTR pCmdLine)	{
	...
	if ( *pCmdLine != '\0' )	{
		...
		if ( *PathFindExtension(str.c_str()) == '\0' )	{
			...
			str.append(L".txt"); // If joined path has no extension, Notepad adds a .txt extension
		}
...
/* generic_string getEasterEggNameFromParam(ParamVector & params, unsigned char & type)
{
	...
	generic_string percentTwentyStr = L"%20";
	generic_string spaceStr = L" ";
	size_t start_pos = 0;
...
const TCHAR FLAG_NOSESSION[] = L"-nosession";
const TCHAR FLAG_NOTABBAR[] = L"-notabbar";
const TCHAR FLAG_SYSTRAY[] = L"-systemtray";
const TCHAR FLAG_LOADINGTIME[] = L"-loadingTime";
const TCHAR FLAG_HELP[] = L"--help";
const TCHAR FLAG_ALWAYS_ON_TOP[] = L"-alwaysOnTop";
const TCHAR FLAG_OPENSESSIONFILE[] = L"-openSession";
const TCHAR FLAG_RECURSIVE[] = L"-r";
const TCHAR FLAG_FUNCLSTEXPORT[] = L"-export=functionList";
const TCHAR FLAG_PRINTANDQUIT[] = L"-quickPrint";
const TCHAR FLAG_NOTEPAD_COMPATIBILITY[] = L"-notepadStyleCmdline";
const TCHAR FLAG_OPEN_FOLDERS_AS_WORKSPACE[] = L"-openFoldersAsWorkspace";
...
void doException(Notepad_plus_Window & notepad_plus_plus)	{
	...
	emergencySavedDir += L"\\N++RECOV";

	bool res = notepad_plus_plus.emergency(emergencySavedDir);
	if (res)	{

		generic_string displayText = L"Notepad++ was able to successfully recover some unsaved documents, or nothing to be saved could be found.\r\nYou can find the results at :\r\n";
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR pCmdLine, int)	{
	...
	if (cmdLineParams._localizationPath[0])
		param.setStartWithLocFileName(cmdLineParams._localizationPath);
	...
	::SetCurrentDirectory(param.getNppPath().c_str());	//force working directory to path of module, preventing lock
...
/* 	TODO: detect update frequency
	...
	/* if (TheFirstOne && isUpExist && isGtXP && isSignatureOK)	{
		...
		if (doUpdatePluginList)	{
			...
			upPlParams += param.getPluginConfDir();
	...
	try
	{
		...
		allowWmCopydataMessages(notepad_plus_plus, param, ver);
		bool going = true;
		while (going)	{
