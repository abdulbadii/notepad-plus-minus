...
LRESULT Notepad_plus::process(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam){
	...
	switch (message)	{
			...
			switch (pCopyData->dwData)	{
				...
				case COPYDATA_PARAMS:	{
					...
					if (sizeof(CmdLineParamsDTO) == cmdLineParamsSize)	{ // make sure the structure is the same
						...
						param.setCmdlineParam(*cmdLineParam);
					}
					else	{
		...
		case NPPM_INTERNAL_SAVECURRENTSESSION:	{
			...
			if (nGUI._rememberLastSession && !nGUI._isCmdlineNosessionActivated)	{

				Session currentSession;
				getCurrentOpenedFiles(currentSession, true);
				param.writeSession(currentSession);
			}
			return TRUE;
		}
		...
		case WM_MOUSEWHEEL:	{
			...
			if ((LOWORD(wParam) & MK_RBUTTON))	{
				...
				param._isTaskListRBUTTONUP_Active = true;
		...
		case NPPM_GETNBSESSIONFILES:	{
			...
			if (param.loadSession(session2Load, sessionFileName))
		...
		case NPPM_DECODESCI:	{
			...
			auto length = pSci->f(SCI_GETLENGTH, 0, 0) + 1;
			char* buffer = new char[length];
			pSci->f(SCI_GETTEXT, length, reinterpret_cast<LPARAM>(buffer));
			...
			pSci->f(SCI_EMPTYUNDOBUFFER);

			pSci->f(SCI_SETCODEPAGE);
...
//			_recBuf = _pDocTab->getBufferByIndex(_pDocTab->getCurrentTabIndex());
		...
		case WM_MACRODLGRUNMACRO:	{
			...
			if (!_recordingMacro)	{ // if we're not currently recording, then playback the recorded keystrokes
				...
				int counter = 0;
				int lastLine = static_cast<int32_t>(_pEditView->f(SCI_GETLINECOUNT)) - 1;
				...
				if (indexMacro != -1)	{
					...
					vector<MacroShortcut> & ms = param.getMacroList();
				...
				for (;;)	{
					...
					if ( times >= 0 )	{
						...
						deltaLastLine = static_cast<int32_t>(_pEditView->f(SCI_GETLINECOUNT)) - 1 - lastLine;
...
/* 		case NPPM_SETSTATUSBAR:	{
		...
		case NPPM_GETMENUHANDLE:	{
			...
			return wParam == NPPPLUGINMENU ? (LRESULT)_pluginsManager.getMenuHandle()
			: wParam == NPPMAINMENU ? (LRESULT)_mainMenuHandle
			: NULL;
		}
		...
		case NPPM_INTERNAL_CLEARSCINTILLAKEY:	{
			...
			_mainEditView.f(SCI_CLEARCMDKEY, wParam);
			_subEditView.f(SCI_CLEARCMDKEY, wParam);
			return TRUE;
		}
		case NPPM_INTERNAL_BINDSCINTILLAKEY:	{

			_mainEditView.f(SCI_ASSIGNCMDKEY, wParam, lParam);
			_subEditView.f(SCI_ASSIGNCMDKEY, wParam, lParam);
			return TRUE;
		}
		...
		case NPPM_INTERNAL_SETCARETWIDTH:	{
			...
			if (nGUI._caretWidth < 4)	{

				_mainEditView.f(SCI_SETCARETSTYLE, CARETSTYLE_LINE);
				_subEditView.f(SCI_SETCARETSTYLE, CARETSTYLE_LINE);
				_mainEditView.f(SCI_SETCARETWIDTH, nGUI._caretWidth);
				_subEditView.f(SCI_SETCARETWIDTH, nGUI._caretWidth);
			}
			else	{

				_mainEditView.f(SCI_SETCARETWIDTH, 1);
				_subEditView.f(SCI_SETCARETWIDTH, 1);
				_mainEditView.f(SCI_SETCARETSTYLE, CARETSTYLE_BLOCK);
				_subEditView.f(SCI_SETCARETSTYLE, CARETSTYLE_BLOCK);
			}
			return TRUE;
		}
		...
		case NPPM_SETSMOOTHFONT:	{
			...
			_mainEditView.f(SCI_SETFONTQUALITY, param);
			_subEditView.f(SCI_SETFONTQUALITY, param);
			return TRUE;
		}
		...
		case NPPM_INTERNAL_SCROLLBEYONDLASTLINE:	{
			...
			const bool endAtLastLine = not (param.getSVP())._scrollBeyondLastLine;
			_mainEditView.f(SCI_SETENDATLASTLINE, endAtLastLine);
			_subEditView.f(SCI_SETENDATLASTLINE, endAtLastLine);
			return TRUE;
		}
		...
		case NPPM_INTERNAL_SETMULTISELCTION:	{
			...
			_mainEditView.f(SCI_SETMULTIPLESELECTION, nGUI._enableMultiSelection);
			_subEditView.f(SCI_SETMULTIPLESELECTION, nGUI._enableMultiSelection);
			return TRUE;
		}

		case NPPM_INTERNAL_SETCARETBLINKRATE:	{

			_mainEditView.f(SCI_SETCARETPERIOD, nGUI._caretBlinkRate);
			_subEditView.f(SCI_SETCARETPERIOD, nGUI._caretBlinkRate);
			return TRUE;
		}
		...
		case WM_CONTEXTMENU:	{
			...
			if (param._isTaskListRBUTTONUP_Active)	{

				param._isTaskListRBUTTONUP_Active = false;
			}
			else	{
				...
				if ((HWND(wParam) == _mainEditView.getHSelf()) || (HWND(wParam) == _subEditView.getHSelf()))	{
					...
					std::vector<MenuItemUnit>& tmp = param.getContextMenuItems();
		...
		case NPPM_INTERNAL_CHECKDOCSTATUS:	{
			...
			if (nGUI._fileAutoDetection != cdDisabled)	{

				bool bCheckOnlyCurrentBuffer = (nGUI._fileAutoDetection & cdEnabledNew) ? true : false;
		...
		case NPPM_INTERNAL_ENABLECHECKDOCOPT:	{
			...
			NppGUI& nGUI = const_cast<NppGUI&>((nppGUI));
			if (wParam == CHECKDOCOPT_NONE)
				...
				nGUI._fileAutoDetection = (cdEnabledOld | cdGo2end | cdAutoUpdate);
		...
		case WM_CLOSE:	{
			...
			if (_pPublicInterface->isPrelaunch())	{
				...
				if (nGUI._cloudPath != L""&& param.isCloudPathChanged())	{

					bool isOK = param.writeSettingsFilesOnCloudForThe1stTime(nGUI._cloudPath);
					if (!isOK)	{

						_nativeLangSpeaker.messageBox("SettingsOnCloudError",
							hwnd,
							L"It seems the path of settings on cloud is set on a read only drive,\ror on a folder needed privilege right for writting access.\rYour settings on cloud will be canceled. Please reset a coherent value via Preference dialog.",
							L"Settings on Cloud",
							MB_OK | MB_APPLMODAL);
				...
				generic_string updaterFullPath = param.getWingupFullPath();
				if (!updaterFullPath.empty())	{

					Process updater(updaterFullPath.c_str(), param.getWingupParams().c_str(), param.getWingupDir().c_str());
					updater.run(param.shouldDoUAC());
				}
			}
			return TRUE;
		}
		...
		case WM_SYSCOMMAND:	{
			...
			if (((nGUI._isMinimizedToTray && !_isAdministrator) || _pPublicInterface->isPrelaunch()) && (wParam == SC_MINIMIZE))	{
		...
		case NPPM_GETENABLETHEMETEXTUREFUNC:	{
			...
			return (LRESULT)param.getEnableThemeDlgTexture();
		}
		...
		case NPPM_GETEDITORDEFAULTBACKGROUNDCOLOR:	{
			...
			return (message == NPPM_GETEDITORDEFAULTFOREGROUNDCOLOR
					?param.getCurrentDefaultFgColor()
					:param.getCurrentDefaultBgColor());
		}
		...
		case NPPM_GETAPPDATAPLUGINSALLOWED:	{ 
			...
			const TCHAR *appDataNpp = param.getAppDataNppDir();
		...
		case NPPM_INTERNAL_SETTING_HISTORY_SIZE:	{
			...
			_lastRecentFileList.setUserMaxNbLRF(param.getNbMaxRecentFile());
			break;
		}
		...
		case NPPM_INTERNAL_SETTING_EDGE_SIZE:	{
			...
			_mainEditView.f(SCI_SETEDGECOLUMN, svp._edgeNbColumn);
			_subEditView.f(SCI_SETEDGECOLUMN, svp._edgeNbColumn);
			break;
		}
		...
		case WM_ENTERMENULOOP:	{
			...
			if (!nGUI._menuBarShow && !wParam && !_sysMenuEntering)
				::SetMenu(hwnd, _mainMenuHandle);
