...
bool FunctionCallTip::getCursorFunction()	{
	...
	if (curValue.lastFunctionIdentifier > -1)	{
		...
		if (_funcName)	{
			...
			if (_ignoreCase)
				same = !testNameNoCase(_funcName, funcToken.token, lstrlen(_funcName));
			else
				same = !generic_strncmp(_funcName, funcToken.token, lstrlen(_funcName));
		}
		if (!same)	{
...
void FunctionCallTip::showCalltip()	{ 
	...
	callTipText << _retVals.at(_currentOverload) << L' ' << _funcName << L' ' << _start;
