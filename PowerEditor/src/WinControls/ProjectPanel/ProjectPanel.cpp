		...
		if (res == IDYES)	{
			...
			if (!saveWorkSpace())
				pNativeSpeaker->messageBox("ProjectPanelChangedSaveError",
					_hSelf,
					L"Your workspace has not been saved.",
					L"$STR_REPLACE$",
					MB_OK | MB_ICONERROR,
					0,
					title);
	...
	if ((notification->hwndFrom == _treeView.getHSelf()))	{
		...
		switch (notification->code)	{
			...
			case TVN_GETINFOTIP:	{
				...
				lpGetInfoTip->cchTextMax = int(str->size());
			}
			break;
...
void ProjectPanel::setFileExtFilter(FileDialog & fDlg)	{
	...
	const TCHAR *ext = nGUI._definedWorkspaceExt.c_str();
	generic_string workspaceExt = L"";
	if (*ext != '\0')	{

		if (*ext != '.')
			workspaceExt += L".";
		workspaceExt += ext;
		fDlg.setExtFilter(L"Workspace file", workspaceExt.c_str(), NULL);
	}
...
int FileRelocalizerDlg::doDialog(const TCHAR *fn, bool isRTL)	{ 
	...
	if (isRTL)	{
		...
		int result = int(::DialogBoxIndirectParam(_hInst, pMyDlgTemplate, _hParent, dlgProc, reinterpret_cast<LPARAM>(this)));
		::GlobalFree(hMyDlgTemplate);
		return result;
	}
	return int(::DialogBoxParam(_hInst, MAKEINTRESOURCE(IDD_FILERELOCALIZER_DIALOG), _hParent, dlgProc, reinterpret_cast<LPARAM>(this)));
}
