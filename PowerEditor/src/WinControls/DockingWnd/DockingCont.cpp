...
DockingCont::DockingCont()
{
	...
	_captionHeightDynamic = param._dpiManager.scaleY(_captionHeightDynamic);
	_captionGapDynamic = param._dpiManager.scaleY(_captionGapDynamic);
	_closeButtonPosLeftDynamic = param._dpiManager.scaleX(_closeButtonPosLeftDynamic);
	_closeButtonPosTopDynamic = param._dpiManager.scaleY(_closeButtonPosTopDynamic);

	_closeButtonWidth = param._dpiManager.scaleX(12); // bitmap image is 12x12
	_closeButtonHeight = param._dpiManager.scaleY(12);
}
...
int DockingCont::getActiveTb()	{
	...
	return int(::SendMessage(_hContTab, TCM_GETCURSEL, 0, 0));
}
...
vector<tTbData*> DockingCont::getDataOfVisTb()
{
	...
	TCITEM tcItem = {0};
	int iItemCnt = int(::SendMessage(_hContTab, TCM_GETITEMCOUNT, 0, 0));
...
void DockingCont::drawCaptionItem(DRAWITEMSTRUCT *pDrawItemStruct)	{
	...
	int length = int(_pszCaption.length());
...
LRESULT DockingCont::runProcTab(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)	{
	...
	switch (Message)	{
		...
		case WM_LBUTTONUP:	{
			...
			iItem = int(::SendMessage(hwnd, TCM_HITTEST, 0, reinterpret_cast<LPARAM>(&info)));
		...
		case WM_MOUSEMOVE:	{
			...
			if ((_beginDrag == TRUE) && (wParam == MK_LBUTTON))	{
				...
				int	iItemSel = int(::SendMessage(hwnd, TCM_GETCURSEL, 0, 0));
...
void DockingCont::drawTabItem(DRAWITEMSTRUCT *pDrawItemStruct)	{
	...
	if (((tTbData*)tcItem.lParam)->uMask & DWS_ICONTAB)	{
		...
		int iPosImage = int(::SendMessage(_hParent, DMM_GETICONPOS, 0, reinterpret_cast<LPARAM>(reinterpret_cast<tTbData*>(tcItem.lParam)->hClient)));
		...
		if ((hImageList != NULL) && (iPosImage >= 0))	{
			...
			int iconDpiDynamicalY = param._dpiManager.scaleY(7);
...
INT_PTR CALLBACK DockingCont::run_dlgProc(UINT Message, WPARAM wParam, LPARAM lParam)	{
	...
	switch (Message)	{ 
		...
		case WM_INITDIALOG:	{
			...
			// set min tab width
			int tabDpiDynamicalMinWidth = param._dpiManager.scaleY(24);
...
void DockingCont::onSize()	{
	...
	RECT rc = {0};
	RECT rcTemp = {0};
	UINT iItemCnt = int(::SendMessage(_hContTab, TCM_GETITEMCOUNT, 0, 0));
	UINT iTabOff = 0;
	...
	if (iItemCnt >= 1)	{
		...
		int tabDpiDynamicalHeight = param._dpiManager.scaleY(24);
