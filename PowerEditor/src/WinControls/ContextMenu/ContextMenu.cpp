...
void ContextMenu::create(HWND hParent, const std::vector<MenuItemUnit> & menuItemArray, const HMENU mainMenuHandle)	{
	...
	for (size_t i = 0, len = menuItemArray.size(); i < len; ++i)	{
		...
		if (mainMenuHandle)	{
			...
			bool isEnabled = !(::GetMenuState(mainMenuHandle, item._cmdID, MF_BYCOMMAND)&(MF_DISABLED|MF_GRAYED));
			bool isChecked = ::GetMenuState(mainMenuHandle, item._cmdID, MF_BYCOMMAND)&(MF_CHECKED);
			if (!isEnabled)
