...
void WordStyleDlg::updateGlobalOverrideCtrls()	{
	...
	::SendDlgItemMessage(_hSelf, IDC_GLOBAL_FG_CHECK, BM_SETCHECK, nGUI._globalOverride.enableFg, 0);
	::SendDlgItemMessage(_hSelf, IDC_GLOBAL_BG_CHECK, BM_SETCHECK, nGUI._globalOverride.enableBg, 0);
	::SendDlgItemMessage(_hSelf, IDC_GLOBAL_FONT_CHECK, BM_SETCHECK, nGUI._globalOverride.enableFont, 0);
	::SendDlgItemMessage(_hSelf, IDC_GLOBAL_FONTSIZE_CHECK, BM_SETCHECK, nGUI._globalOverride.enableFontSize, 0);
	::SendDlgItemMessage(_hSelf, IDC_GLOBAL_BOLD_CHECK, BM_SETCHECK, nGUI._globalOverride.enableBold, 0);
	::SendDlgItemMessage(_hSelf, IDC_GLOBAL_ITALIC_CHECK, BM_SETCHECK, nGUI._globalOverride.enableItalic, 0);
	::SendDlgItemMessage(_hSelf, IDC_GLOBAL_UNDERLINE_CHECK, BM_SETCHECK, nGUI._globalOverride.enableUnderLine, 0);
}
	...
	switch (Message)	{
		...
		case WM_INITDIALOG :	{
			...
			for (size_t i = 0 ; i < themeSwitcher.size() ; ++i)	{
				...
				int j = int(::SendMessage(_hSwitch2ThemeCombo, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(themeInfo.first.c_str())));
				if (! themeInfo.second.compare(nGUI._themeName) )	{
		...
		case WM_HSCROLL :	{
			...
			if (reinterpret_cast<HWND>(lParam) == ::GetDlgItem(_hSelf, IDC_SC_PERCENTAGE_SLIDER))	{
				...
				int percent = int(::SendDlgItemMessage(_hSelf, IDC_SC_PERCENTAGE_SLIDER, TBM_GETPOS, 0, 0));
				...
				switch (wParam)	{
					...
					case IDCANCEL :
						if (_isDirty)	{
							...
							if (_restoreInvalid)	{
								...
								generic_string str( nGUI._themeName );
								param.reloadStylers( &str[0] );
							}

							LexerStylerArray & lsArray = param.getLStylerArray();
					...
					case IDC_SAVECLOSE_BUTTON :	{
						...
						if (_isDirty)	{
							...
							_currentThemeIndex = int(::SendMessage(_hSwitch2ThemeCombo, CB_GETCURSEL, 0, 0));
					...
					case IDC_GLOBAL_FG_CHECK :	{
						...
						GlobalOverride & glo = param.getGlobalOverrideStyle();
						glo.enableFg = (BST_CHECKED == ::SendDlgItemMessage(_hSelf, int(wParam), BM_GETCHECK, 0, 0));
						notifyDataModified();
						apply();
						return TRUE;
					}
					...
					case  IDC_GLOBAL_BG_CHECK:	{
						...
						GlobalOverride & glo = param.getGlobalOverrideStyle();
						glo.enableBg = (BST_CHECKED == ::SendDlgItemMessage(_hSelf, int(wParam), BM_GETCHECK, 0, 0));
						notifyDataModified();
						apply();
						return TRUE;
					}
					...
					case IDC_GLOBAL_FONT_CHECK :	{
						...
						GlobalOverride & glo = param.getGlobalOverrideStyle();
						glo.enableFont = (BST_CHECKED == ::SendDlgItemMessage(_hSelf, int(wParam), BM_GETCHECK, 0, 0));
						notifyDataModified();
						apply();
						return TRUE;
					}
					case IDC_GLOBAL_FONTSIZE_CHECK :	{
						...
						GlobalOverride & glo = param.getGlobalOverrideStyle();
						glo.enableFontSize = (BST_CHECKED == ::SendDlgItemMessage(_hSelf, int(wParam), BM_GETCHECK, 0, 0));
						notifyDataModified();
						apply();
						return TRUE;
					}
					case IDC_GLOBAL_BOLD_CHECK :	{
						...
						GlobalOverride & glo = param.getGlobalOverrideStyle();
						glo.enableBold = (BST_CHECKED == ::SendDlgItemMessage(_hSelf, int(wParam), BM_GETCHECK, 0, 0));
						notifyDataModified();
						apply();
						return TRUE;
					}
					...
					case IDC_GLOBAL_ITALIC_CHECK :	{
						...
						GlobalOverride & glo = param.getGlobalOverrideStyle();
						glo.enableItalic = (BST_CHECKED == ::SendDlgItemMessage(_hSelf, int(wParam), BM_GETCHECK, 0, 0));
						notifyDataModified();
						apply();
						return TRUE;
					}
					case IDC_GLOBAL_UNDERLINE_CHECK :	{
						...
						GlobalOverride & glo = param.getGlobalOverrideStyle();
						glo.enableUnderLine = (BST_CHECKED == ::SendDlgItemMessage(_hSelf, int(wParam), BM_GETCHECK, 0, 0));
						notifyDataModified();
						apply();
						return TRUE;
					}
							...
							case CBN_SELCHANGE :	{ // == case LBN_SELCHANGE :
								...
								switch (LOWORD(wParam))	{
									...
									case IDC_LANGUAGES_LIST :	{
										...
										int i = int(::SendDlgItemMessage(_hSelf, LOWORD(wParam), LB_GETCURSEL, 0, 0));
										if (i != LB_ERR)	{
...
void WordStyleDlg::loadLangListFromNppParam()	{
	...
	_lsArray = param.getLStylerArray();
	_globalStyles = param.getGlobalStylers();
...
void WordStyleDlg::switchToTheme()	{
	...
	param.reloadStylers(&_themeName[0]);
...
void WordStyleDlg::setStyleListFromLexer(int index)	{
	...
	if (index)	{
		...
		const TCHAR *ext = param.getLangExtFromName(langName);
...
void WordStyleDlg::setVisualFromStyleList()	{
	...
	if (shouldBeDisplayed)	{
		...
		LangType lType = param.getLangIDFromStr(lexerStyler.getLexerName());
		if (lType == L_TEXT)	{
		...
		const TCHAR *kws = param.getWordList(lType, style._keywordClass);
		if (!kws)
			kws = L"";
		::SendDlgItemMessage(_hSelf, IDC_DEF_KEYWORDS_EDIT, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(kws));

		const TCHAR *ckwStr = (style._keywords)?style._keywords->c_str():L"";
