...
void TreeView::init(HINSTANCE hInst, HWND pere)
{
	...
	Parent = insertTo(Parent, L"Native command", 0);
	insertTo(Parent, L"Power On", 0);
	insertTo(Parent, L"Power off", 0);
	insertTo(Parent, L"Entrant", 0);
	insertTo(Parent, L"Sortant", 0);
	Parent = insertTo(Before, L"Macro", 0);
	insertTo(Parent, L"ChangeCode", 0);
	insertTo(Parent, L"CipherData", 0);

	insertTo(NULL, L"Bla bla bla bla...", 0);
	//display();
}