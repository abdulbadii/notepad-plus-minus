...
void ShortcutMapper::initTabs()	{
	...
	wp.rcNormalPosition.bottom = param._dpiManager.scaleY(30);
...
void ShortcutMapper::getClientRect(RECT & rc) const 
{
		...
		rc.top += param._dpiManager.scaleY(30);
		rc.bottom -= param._dpiManager.scaleY(108);
		rc.left += param._dpiManager.scaleX(5);
		rc.right -= param._dpiManager.scaleX(5);
}
...
void ShortcutMapper::initBabyGrid()	{
	...
	_hGridFonts.at(GFONT_HEADER) = ::CreateFont(
		param._dpiManager.scaleY(18), 0, 0, 0, FW_BOLD,
	...
	_hGridFonts.at(GFONT_ROWS) = ::CreateFont(
		param._dpiManager.scaleY(16), 0, 0, 0, FW_NORMAL,
		FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH,
		L"MS Shell Dlg");
	...
	_babygrid.setColWidth(0, param._dpiManager.scaleX(30));  // Force the first col to be small, others col will be automatically sized
	_babygrid.setHeaderHeight(param._dpiManager.scaleY(21));
	_babygrid.setRowHeight(param._dpiManager.scaleY(21));
...
void ShortcutMapper::fillOutBabyGrid()	{
	...
	size_t nbItems = 0;
	NativeLangSpeaker* nativeLangSpeaker = param.getNativeLangSpeaker();
	generic_string nameStr = nativeLangSpeaker->getShortcutMapperLangStr("ColumnName", L"Name");
	generic_string shortcutStr = nativeLangSpeaker->getShortcutMapperLangStr("ColumnShortcut", L"Shortcut");
	...
	switch(_currentState)	{
		...
		case STATE_MENU:	{
			...
			nbItems = param.getUserShortcuts().size();
			_babygrid.setLineColNumber(nbItems, 3);
			generic_string categoryStr = nativeLangSpeaker->getShortcutMapperLangStr("ColumnCategory", L"Category");
		...
		case STATE_MACRO:	{
			...
			nbItems = param.getMacroList().size();
		...
		case STATE_USER:	{
			...
			nbItems = param.getUserCommandList().size();
			_babygrid.setLineColNumber(nbItems, 2);
		}
		break;
		...
		case STATE_PLUGIN:	{
			...
			nbItems = param.getPluginCommandList().size();
			_babygrid.setLineColNumber(nbItems, 3);
			generic_string pluginStr = nativeLangSpeaker->getShortcutMapperLangStr("ColumnPlugin", L"Plugin");
		...
		case STATE_SCINTILLA:	{
			...
			nbItems = param.getScintillaKeyList().size();
			_babygrid.setLineColNumber(nbItems, 2);
		}
		break;
	}
	...
	switch(_currentState)	{ 
		...
		case STATE_MENU:	{
			...
			vector<CommandShortcut> & cshortcuts = param.getUserShortcuts();
			cs_index = 1;
		...
		case STATE_MACRO:	{
			...
			vector<MacroShortcut> & cshortcuts = param.getMacroList();
			cs_index = 1;
			for (size_t i = 0; i < nbItems; ++i)	{
		...
		case STATE_USER:	{
			...
			vector<UserCommand> & cshortcuts = param.getUserCommandList();
			cs_index = 1;
			for (size_t i = 0; i < nbItems; ++i)	{
		...
		case STATE_PLUGIN:	{
			...
			vector<PluginCmdShortcut> & cshortcuts = param.getPluginCommandList();
			cs_index = 1;
			for (size_t i = 0; i < nbItems; ++i)	{
		...
		case STATE_SCINTILLA:	{
			...
			vector<ScintillaKeyMap> & cshortcuts = param.getScintillaKeyList();
			cs_index=1;
			for (size_t i = 0; i < nbItems; ++i)	{
				...
				case IDM_BABYGRID_CLEAR :	{
					...
					switch(_currentState)	{
						...
						case STATE_MENU:	{
							...
							param.addUserModifiedIndex(shortcutIndex);
						...
						case STATE_MACRO:	{ 
							...
							vector<MacroShortcut> & shortcuts = param.getMacroList();
						...
						case STATE_USER:	{ 
							...
							vector<UserCommand> & shortcuts = param.getUserCommandList();
						...
						case STATE_PLUGIN:	{ 
							...
							vector<PluginCmdShortcut> & shortcuts = param.getPluginCommandList();
							PluginCmdShortcut pcsc = shortcuts[shortcutIndex];
							pcsc.clear();
							//shortcut was altered
							param.addPluginModifiedIndex(shortcutIndex);
				...
				case IDM_BABYGRID_MODIFY :	{
					...
					switch(_currentState)	{
						...
						case STATE_SCINTILLA:	{
							...
							vector<ScintillaKeyMap> & shortcuts = param.getScintillaKeyList();
							...
							if (skm.doDialog() != -1 && prevskm != skm)	{
								...
								//shortcut was altered
								param.addScintillaModifiedIndex((int)shortcutIndex);
								...
								param.getScintillaAccelerator()->updateKeys();
								param.setShortcutDirty();
							}
						}
						break;  
					...
					if (res == IDOK)	{
						...
						switch(_currentState)	{ 
							...
							case STATE_USER:	{ 
								...
								vector<UserCommand> & theUserCmds = param.getUserCommandList();
					...
					switch (HIWORD(wParam))	{
						...
						case BGN_CELLRCLICKED:	{ //a cell was clicked in the properties grid
							...
							if (!_rightClickMenu.isCreated())	{
								...
								itemUnitArray.push_back(MenuItemUnit(IDM_BABYGRID_MODIFY, L"Modify"));
								itemUnitArray.push_back(MenuItemUnit(IDM_BABYGRID_DELETE, L"Delete"));
								itemUnitArray.push_back(MenuItemUnit(IDM_BABYGRID_CLEAR, L"Clear"));
						...
						case BGN_ROWCHANGED:	{
							...
							switch (_currentState)	{
								...
								case STATE_MENU:	{
									...
									vector<CommandShortcut> & vShortcuts = param.getUserShortcuts();
								...
								case STATE_MACRO:	{
									...
									vector<MacroShortcut> & vShortcuts = param.getMacroList();
									findKeyConflicts(&conflictInfo, vShortcuts[currentIndex].getKeyCombo(), currentIndex);
								}
								break;
								...
								case STATE_USER:	{
									...
									vector<UserCommand> & vShortcuts = param.getUserCommandList();
									findKeyConflicts(&conflictInfo, vShortcuts[currentIndex].getKeyCombo(), currentIndex);
								}
								break;
								...
								case STATE_PLUGIN:	{
									...
									vector<PluginCmdShortcut> & vShortcuts = param.getPluginCommandList();
									findKeyConflicts(&conflictInfo, vShortcuts[currentIndex].getKeyCombo(), currentIndex);
								}
								break;
								...
								case STATE_SCINTILLA:	{
									...
									vector<ScintillaKeyMap> & vShortcuts = param.getScintillaKeyList();
