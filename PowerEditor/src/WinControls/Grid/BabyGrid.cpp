...
int GetNextColWithWidth(int SI, int startcol, int direction)
	...
	while((j>0)&&(j<=BGHS[SI].cols)&&(!BGHS[SI].columnwidths[j]));
...
void DisplayColumn(HWND hWnd,int SI,int c,int offset,HFONT hfont,HFONT hcolumnheadingfont)
{
	...
	if(!BGHS[SI].columnwidths[c]){return;}
		...
		case BGM_SETCELLDATA:
			...
			FindResult = static_cast<int32_t>(SendMessage(BGHS[SelfIndex].hlist1, LB_ADDSTRING, 0, reinterpret_cast<LPARAM>(buffer)));
				...
				//get the last line and adjust grid dimmensions
						...
						if((BGHS[SelfIndex].COLAUTOWIDTH)||(!LPBGcell->row))
							{
							HDC hdc;
							SIZE size { 0, 0 };
							int required_width;
							...
							//count lines
								...
								wcscpy_s(tbuffer,(TCHAR*)lParam);
										...
										if((!BGHS[SelfIndex].ELLIPSIS)||(!LPBGcell->row))
											{
				...
				DrawCursor(hWnd,SelfIndex);

							{
								...
								if(wParam == 8) //backspace
										...
										BGHS[SelfIndex].EDITING = TRUE;
										if(!lstrlen(BGHS[SelfIndex].editstring))
											{
			...
			hInst = lpcs->hInstance;
				...
				if(CountGrids()==1)
						...
						hfontbody=CreateFont(16,0,0, 0,
										...
										0 ,
										L"MS Shell Dlg");
						hfontheader=CreateFont(18,0,0, 0,FW_HEAVY,FALSE,FALSE,FALSE,DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, 0, 0, L"MS Shell Dlg");
						hfonttitle=CreateFont(20,0,0, 0,FW_HEAVY,FALSE,FALSE,FALSE,DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, 0, 0, L"MS Shell Dlg");
						}
