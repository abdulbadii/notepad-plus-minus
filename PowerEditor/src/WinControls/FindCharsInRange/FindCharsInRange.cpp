		...
		case WM_COMMAND :	{
			...
			switch (wParam)	{
				...
				case ID_FINDCHAR_NEXT:	{
					...
					int currentPos = static_cast<int32_t>((*_ppEditView)->f(SCI_GETCURRENTPOS));
					...
					if (!getRangeFromUI(startRange, endRange))	{
						...
						//STOP!
						param.getNativeLangSpeaker()->messageBox("FindCharRangeValueError",
							_hSelf,
...
bool FindCharsInRangeDlg::findCharInRange(unsigned char beginRange, unsigned char endRange, int startPos, bool direction, bool wrap)	{
	...
	if (found != -1)	{
		...
		//printInt(found);
		auto sci_line = (*_ppEditView)->f(SCI_LINEFROMPOSITION, found);
		(*_ppEditView)->f(SCI_ENSUREVISIBLE, sci_line);
		(*_ppEditView)->f(SCI_GOTOPOS, found);
		(*_ppEditView)->f(SCI_SETSEL, (direction == dirDown)?found:found+1, (direction == dirDown)?found+1:found);
	}
	delete [] content;
	return (found != -1);
}
