...
//Menu
TCHAR szNppName[] = L"notepad++.exe";
TCHAR szDefaultMenutext[] = L"Edit with &Notepad++";

#ifdef WIN64
TCHAR szShellExtensionTitle[] = L"ANotepad++64";
TCHAR szShellExtensionKey[] = L"*\\shellex\\ContextMenuHandlers\\ANotepad++64";
#else
TCHAR szShellExtensionTitle[] = L"ANotepad++";
TCHAR szShellExtensionKey[] = L"*\\shellex\\ContextMenuHandlers\\ANotepad++";
#endif
...
TCHAR szDefaultCustomcommand[] = L"";
//Icon
DWORD isDynamic = 1;
DWORD maxText = 25;
DWORD showIcon = 1;
...
STDAPI DllInstall(BOOL bInstall, LPCWSTR /*pszCmdLine*/) {
	if (bInstall) {
		...
		MsgBoxError(L"Uninstalling not supported, use DllUnregisterServer instead");
		return E_NOTIMPL;
	}
}
...
BOOL RegisterServer() {
	...
	static DOREGSTRUCT ClsidEntries[] = {
		...
		//Settings
		// Context menu
		{HKEY_CLASSES_ROOT,	L"CLSID\\%s\\Settings",						L"Title",			REG_SZ,		szDefaultMenutext},
		{HKEY_CLASSES_ROOT,	L"CLSID\\%s\\Settings",						L"Path",			REG_SZ,		szDefaultPath},
		{HKEY_CLASSES_ROOT,	L"CLSID\\%s\\Settings",						L"Custom",			REG_SZ,		szDefaultCustomcommand},
		{HKEY_CLASSES_ROOT,	L"CLSID\\%s\\Settings",						L"ShowIcon",		REG_DWORD,	(LPTSTR)&showIcon},
		// Icon
		{HKEY_CLASSES_ROOT,	L"CLSID\\%s\\Settings",						L"Dynamic",		REG_DWORD,	(LPTSTR)&isDynamic},
		{HKEY_CLASSES_ROOT,	L"CLSID\\%s\\Settings",						L"Maxtext",		REG_DWORD,	(LPTSTR)&maxText},

		//Registration
		// Context menu
		{HKEY_CLASSES_ROOT,	szShellExtensionKey,	NULL,					REG_SZ,		szGUID},
		// Icon
		//{HKEY_CLASSES_ROOT,	L"Notepad++_file\\shellex\\IconHandler",		NULL,					REG_SZ,		szGUID},
...
BOOL UnregisterServer() {
	...
	wsprintf(szKeyTemp, L"Notepad++_file\\shellex\\IconHandler");
	RegDeleteKey(HKEY_CLASSES_ROOT, szKeyTemp);
	wsprintf(szKeyTemp, L"Notepad++_file\\shellex");
	RegDeleteKey(HKEY_CLASSES_ROOT, szKeyTemp);

	wsprintf(szKeyTemp, L"CLSID\\%s\\InprocServer32", szGUID);
	RegDeleteKey(HKEY_CLASSES_ROOT, szKeyTemp);
	wsprintf(szKeyTemp, L"CLSID\\%s\\Settings", szGUID);
	RegDeleteKey(HKEY_CLASSES_ROOT, szKeyTemp);
	wsprintf(szKeyTemp, L"CLSID\\%s", szGUID);
	RegDeleteKey(HKEY_CLASSES_ROOT, szKeyTemp);
	...
	switch(uMsg) {
		case WM_INITDIALOG: {
			...
			if (result == ERROR_SUCCESS) {
				...
				result = RegQueryValueEx(settingKey, L"Title", NULL, NULL, (LPBYTE)(customText), &size);
				if (result != ERROR_SUCCESS) {
					lstrcpyn(customText, szDefaultMenutext, TITLE_SIZE);
				}

				size = sizeof(TCHAR)*MAX_PATH;
				result = RegQueryValueEx(settingKey, L"Custom", NULL, NULL, (LPBYTE)(customCommand), &size);
				if (result != ERROR_SUCCESS) {
					lstrcpyn(customCommand, L"", MAX_PATH);
				}

				size = sizeof(DWORD);
				result = RegQueryValueEx(settingKey, L"Dynamic", NULL, NULL, (BYTE*)(&isDynamic), &size);
				if (result != ERROR_SUCCESS) {
					isDynamic = 1;
				}

				size = sizeof(DWORD);
				result = RegQueryValueEx(settingKey, L"ShowIcon", NULL, NULL, (BYTE*)(&useMenuIcon), &size);
				if (result != ERROR_SUCCESS) {
					useMenuIcon = 1;
				}
			...
			switch(LOWORD(wParam)) {
				case IDOK: {
					...
					if (result == ERROR_SUCCESS) {
						...
						result = RegSetValueEx(settingKey, L"Title", 0,REG_SZ, (LPBYTE)customText, (textLen+1)*sizeof(TCHAR));
						result = RegSetValueEx(settingKey, L"Custom", 0,REG_SZ, (LPBYTE)customCommand, (commandLen+1)*sizeof(TCHAR));

						result = RegSetValueEx(settingKey, L"Dynamic", 0, REG_DWORD, (LPBYTE)&isDynamic, sizeof(DWORD));
						result = RegSetValueEx(settingKey, L"ShowIcon", 0, REG_DWORD, (LPBYTE)&useMenuIcon, sizeof(DWORD));
					...
					if (showIcon == 1) {
						result = RegCreateKeyEx(HKEY_CLASSES_ROOT, L"Notepad++_file\\shellex\\IconHandler", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &settingKey, NULL);
						if (result == ERROR_SUCCESS) {
							result = RegSetValueEx(settingKey, NULL, 0,REG_SZ, (LPBYTE)szGUID, (lstrlen(szGUID)+1)*sizeof(TCHAR));
							RegCloseKey(settingKey);
						}
	...
	m_hBitmap(NULL)
{
	...
	if (result == ERROR_SUCCESS) {
		size = sizeof(TCHAR)*TITLE_SIZE;
		result = RegQueryValueEx(settingKey, L"Title", NULL, NULL, (LPBYTE)(m_szMenuTitle), &size);
		if (result != ERROR_SUCCESS) {
		...
		size = sizeof(DWORD);
		result = RegQueryValueEx(settingKey, L"Maxtext", NULL, NULL, (BYTE*)(&siz), &size);
		if (result == ERROR_SUCCESS) {
		...
		if (result == ERROR_SUCCESS) {
			m_useCustom = true;
			size = MAX_PATH;
			RegQueryValueEx(settingKey, L"CustomIcon", NULL, NULL, (BYTE*)m_szCustomPath, &size);
		}
...
STDMETHODIMP CShellExt::GetIconLocation(UINT uFlags, LPTSTR szIconFile, UINT cchMax, int * piIndex, UINT * pwFlags) {
	*pwFlags = 0;
	...
	if(cchMax > 0) {
		lstrcpyn(szIconFile, L"NppShellIcon", cchMax);
...
STDMETHODIMP CShellExt::Extract(LPCTSTR /*pszFile*/, UINT /*nIconIndex*/, HICON * phiconLarge, HICON * phiconSmall, UINT nIconSize) {
	...
	lstrcpyn(lf.lfFaceName, L"Courier New", LF_FACESIZE);
...
STDMETHODIMP CShellExt::InvokeNPP(HWND /*hParent*/, LPCSTR /*pszWorkingDir*/, LPCSTR /*pszCmd*/, LPCSTR /*pszParam*/, int iShowCmd) {
	...
	result = RegOpenKeyEx(HKEY_CLASSES_ROOT, szKeyTemp, 0, KEY_READ, &settingKey);
	if (result != ERROR_SUCCESS) {
		MsgBoxError(L"Unable to open registry key.");
		return E_FAIL;
	}

	result = RegQueryValueEx(settingKey, L"Path", NULL, NULL, NULL, &regSize);
	if (result == ERROR_SUCCESS) {
		bytesRequired += regSize+2;
	} else {
		MsgBoxError(L"Cannot read path to executable.");
		RegCloseKey(settingKey);
		return E_FAIL;
	}

	result = RegQueryValueEx(settingKey, L"Custom", NULL, NULL, NULL, &regSize);
	if (result == ERROR_SUCCESS) {
	...
	*pszCommand = 0;
	...
	result = RegQueryValueEx(settingKey, L"Path", NULL, NULL, (LPBYTE)(szFilename), &regSize);
	szFilename[MAX_PATH-1] = 0;
	lstrcat(szNotepadExecutableFilename, L"\"");
	lstrcat(szNotepadExecutableFilename, szFilename);
	lstrcat(szNotepadExecutableFilename, L"\"");
	result = RegQueryValueEx(settingKey, L"Custom", NULL, NULL, (LPBYTE)(szCustom), &pathSize);
	if (result == ERROR_SUCCESS) {
		lstrcat(szNotepadExecutableFilename, L" ");
	...
	while(iFileIndex < m_cbFiles) {
		...
		if (!CreateProcess (NULL, pszCommand, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
			...
			if (errorCode == ERROR_ELEVATION_REQUIRED) {	//Fallback to shellexecute
				...
				HINSTANCE execVal = ShellExecute(NULL, L"runas", pszCommand, NULL, NULL, iShowCmd);
				CoUninitialize();
				if (execVal <= (HINSTANCE)32) {
					TCHAR * message = new TCHAR[512+bytesRequired];
					wsprintf(message, L"ShellExecute failed (%d): Is this command correct?\r\n%s", execVal, pszCommand);
					MsgBoxError(message);
					delete [] message;
				}
			} else {
				TCHAR * message = new TCHAR[512+bytesRequired];
				wsprintf(message, L"Error in CreateProcess (%d): Is this command correct?\r\n%s", errorCode, pszCommand);
				MsgBoxError(message);
				delete [] message;
			}
		}
