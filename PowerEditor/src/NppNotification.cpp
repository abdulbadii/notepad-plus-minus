...
BOOL Notepad_plus::notify(SCNotification *notification)	{
	...
	switch (notification->nmhdr.code)	{
		...
		case SCN_MODIFIED:	{
			...
			if (notification->modificationType & (SC_PERFORMED_UNDO | SC_PERFORMED_REDO))
				// for the backup system
				_pEditView->getCurrentBuffer()->setModifiedStatus(true);

			if (notification->modificationType & SC_MOD_CHANGEFOLD && prevWasEdit)	{
			...
			if (notification->nmhdr.code == TCN_TABDROPPEDOUTSIDE)	{
				...
				if (hWin == _pEditView->getHSelf())	{ // In the same view group
					...
					if (!_tabPopupDropMenu.isCreated())	{
						...
						TCHAR goToView[32] = L"Move to Other View";
						TCHAR cloneToView[32] = L"Clone to Other View";
		...
		case NM_RCLICK :	{
			...
			if (notification->nmhdr.hwndFrom == _mainDocTab.getHSelf())
				...
				if (_pFileSwitcherPanel->nbSelectedFiles() > 1)	{
					...
					if (!_fileSwitcherMultiFilePopupMenu.isCreated())	{
						...
						vector<MenuItemUnit> itemUnitArray;
						itemUnitArray.push_back(MenuItemUnit(IDM_FILESWITCHER_FILESCLOSE, L"Close Selected files"));
						itemUnitArray.push_back(MenuItemUnit(IDM_FILESWITCHER_FILESCLOSEOTHERS, L"Close others files"));
			...
			if (!_tabPopupMenu.isCreated())	{
				...
				itemUnitArray.push_back(MenuItemUnit(IDM_FILE_CLOSEALL_UNCHANGED, L"Close All Unchanged"));
				...
				itemUnitArray.push_back(MenuItemUnit(IDM_FILE_OPEN_FOLDER, L"Open Containing Folder in Explorer"));
				itemUnitArray.push_back(MenuItemUnit(IDM_FILE_OPEN_CMD, L"Open Containing Folder in cmd"));
				itemUnitArray.push_back(MenuItemUnit(0, NULL));
				itemUnitArray.push_back(MenuItemUnit(IDM_FILE_OPEN_DEFAULT_VIEWER, L"Open in Default Viewer"));
				itemUnitArray.push_back(MenuItemUnit(0, NULL));
				...
				itemUnitArray.push_back(MenuItemUnit(0, NULL));
				itemUnitArray.push_back(MenuItemUnit(IDM_EDIT_FULLPATHTOCLIP,   L"Full File Path to Clipboard"));
				itemUnitArray.push_back(MenuItemUnit(IDM_EDIT_FILENAMETOCLIP,   L"Filename to Clipboard"));
				itemUnitArray.push_back(MenuItemUnit(IDM_EDIT_CURRENTDIRTOCLIP, L"Current Dir. Path to Clipboard"));
				itemUnitArray.push_back(MenuItemUnit(0, NULL));
				itemUnitArray.push_back(MenuItemUnit(IDM_VIEW_GOTO_ANOTHER_VIEW, L"Move to Other View"));
				itemUnitArray.push_back(MenuItemUnit(IDM_VIEW_CLONE_TO_ANOTHER_VIEW, L"Clone to Other View"));
				itemUnitArray.push_back(MenuItemUnit(IDM_VIEW_GOTO_NEW_INSTANCE, L"Move to New Instance"));
				itemUnitArray.push_back(MenuItemUnit(IDM_VIEW_LOAD_IN_NEW_INSTANCE, L"Open in New Instance"));
