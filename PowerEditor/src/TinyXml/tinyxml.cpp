...
void TiXmlAttribute::Print( FILE* cfile, int /*depth*/ ) const
{
	...
	if (value.find ('\"') == TIXML_STRING::npos)
		generic_fprintf (cfile, L"%s=\"%s\"", n.c_str(), v.c_str() );
	else
		generic_fprintf (cfile, L"%s='%s'", n.c_str(), v.c_str() );
}
...
void TiXmlText::Print( FILE* cfile, int /*depth*/ ) const
{
	...
	generic_fprintf( cfile, L"%s", buffer.c_str() );
}
...
void TiXmlDeclaration::Print( FILE* cfile, int /*depth*/ ) const
{
	...
	if ( !version.empty() )
		generic_fprintf (cfile, L"version=\"%s\" ", version.c_str ());
	if ( !encoding.empty() )
		generic_fprintf (cfile, L"encoding=\"%s\" ", encoding.c_str ());
	if ( !standalone.empty() )
		generic_fprintf (cfile, L"standalone=\"%s\" ", standalone.c_str ());
	generic_fprintf (cfile, L"?>");
}
