		...
		switch (waitStatus)	{
				...
				// We've received a notification in the queue.
			{
				...
				while (changes.Pop(dwAction, fn))	{
					...
					if (!OrdinalIgnoreCaseCompareStrings(fullFileName, fn.c_str()))	{
...
BufferID Notepad_plus::doOpen(const generic_string& fileName, bool isRecursive, bool isReadOnly, int encoding, const TCHAR *backupFileName, FILETIME fileNameTimestamp)
{
	...
	if (isFileWorkspace(longFileName) && PathFileExists(longFileName))	{
		...
		param.setWorkSpaceFilePath(0, longFileName);
	...
	if (!isSnapshotMode)	{ // if not backup mode, or backupfile path is invalid
		...
		if (!PathFileExists(longFileName) && !globbing)	{
			...
			if (PathFileExists(longFileDir.c_str()))	{
				...
				generic_string str2display = L"\"";
				str2display += longFileName;
				str2display += L"\" cannot be opened:\nFolder \"";
				str2display += longFileDir;
				str2display += L"\" doesn't exist.";
				::MessageBox(_pPublicInterface->getHSelf(), str2display.c_str(), L"Cannot open file", MB_OK);
			}
...
bool Notepad_plus::doSave(BufferID id, const TCHAR * filename, bool isCopy)	{
		...
		if (!_isAdministrator)	{
			...
			if (isSnapshotMode)	{ // if both rememberSession && backup mode are enabled
				...
				if (openInAdminModeRes == IDYES)	{
					...
					generic_string args = L"-multiInst";
    ...
    // open only the file to save in Notepad++ of Administrator mode by keeping the current instance.
				...
				if (openInAdminModeRes == IDYES)	{
					...
					generic_string args = L"-multiInst -nosession ";
					args += L"\"";
					args += fileNamePath;
					args += L"\"";
					size_t shellExecRes = (size_t)::ShellExecute(_pPublicInterface->getHSelf(), L"runas", nppFullPath, args.c_str(), L".", SW_SHOW);
...
void Notepad_plus::doClose(BufferID id, int whichOne, bool doDeleteBackup){
	...
	if (buf->isMonitoringOn())	// turn off monitoring
...
bool Notepad_plus::fileCloseAll(bool doDeleteBackup, bool isSnapshotMode)	{
	...
	if (bothActive())	{
		...
		for (int32_t i = int(_pNonDocTab->nbItem()) - 1; i >= 0; --i )	{ //close all from right to left
	...
	for (int32_t i = int(_pDocTab->nbItem()) - 1; i >= 0; --i )	{
...
bool Notepad_plus::fileCloseAllButCurrent()	{
	...
	for (size_t i = 0; i < _mainDocTab.nbItem() && !noSaveToAll; ++i)	{
		...
		if (buf->isUntitled() && buf->docLength() == 0)	{
			...
			if (res == IDCANCEL)	{
				...
				for (int32_t j = int(mainSaveOpIndex.size()) - 1; j >= 0; --j )	{ 	//close all from right to left
	...
	for (size_t i = 0; i < _subDocTab.nbItem() && !noSaveToAll; ++i)	{
		...
		if (buf->isUntitled() && buf->docLength() == 0)	{
			...
			if (res == IDCANCEL)	{
				...
				for (int32_t j = int(subSaveOpIndex.size()) - 1; j >= 0; --j )	{ 	//close all from right to left
	...
	for (int32_t i = int(_pDocTab->nbItem()) - 1; i >= 0; --i )	{	//close all from right to left
...
bool Notepad_plus::fileSave(BufferID id)	{
	...
	if (buf->isDirty() && !buf->getFileReadOnly())	{	// save if not readonly
		...
		BackupFeature backup = nGUI._backup;
		...
		if (backup != bak_none)	{
			...
			if (nGUI._useDir && not nGUI._backupDir.empty())	{

				// Get the custom directory, make sure it has a trailing slash
				fn_bak = nGUI._backupDir;
				if (fn_bak.back() != L'\\')
					fn_bak += L"\\";
			}
			else	{
	...
	if (pfn)	{
		...
		if (other == BUFFER_INVALID)	{	//can save, as both (same and other) view don't contain buffer
			...
			_nativeLangSpeaker.messageBox("FileAlreadyOpenedInNpp",
				_pPublicInterface->getHSelf(),
				L"The file is already opened in the Notepad++.",
				L"ERROR",
				MB_OK | MB_ICONSTOP);
			switchToFile(other);
			return false;
...
bool Notepad_plus::fileRename(BufferID id)	{
	...
	if (isFileExisting)	{
		...
		generic_string title = _nativeLangSpeaker.getLocalizedStrFromID("tabrename-title", L"Rename Current Tab");
		generic_string staticName = _nativeLangSpeaker.getLocalizedStrFromID("tabrename-newname", L"New Name: ");
...
bool Notepad_plus::fileDelete(BufferID id)	{
	...
	winVer winVersion = param.getWinVersion();
	bool goAhead = true;
...
bool Notepad_plus::isFileSession(const TCHAR * filename)	{
	...
	const TCHAR *definedSessionExt = nppGUI._definedSessionExt.c_str();
	if (*definedSessionExt != '\0')	{

		generic_string fncp = filename;
		TCHAR *pExt = PathFindExtension(fncp.c_str());

		generic_string usrSessionExt = L"";
...
bool Notepad_plus::isFileWorkspace(const TCHAR * filename) {
	...
	const TCHAR *definedWorkspaceExt = nppGUI._definedWorkspaceExt.c_str();
	if (*definedWorkspaceExt != '\0')	{

		generic_string fncp = filename;
		TCHAR *pExt = PathFindExtension(fncp.c_str());

		generic_string usrWorkspaceExt = L"";
...
void Notepad_plus::loadLastSession()	{
	...
	Session lastSession = param.getSession();
...
bool Notepad_plus::loadSession(Session & session, bool isSnapshotMode)	{
	...
	for (size_t i = 0; i < session.nbMainFiles() ; )	{
		...
		if (PathFileExists(pFn))	{
			...
			if (isSnapshotMode && session._mainViewFiles[i]._backupFilePath != L"")
	...
	if ((session.nbSubFiles() > 0) && (session._activeView == MAIN_VIEW || session._activeView == SUB_VIEW))
		switchEditViewTo(int(session._activeView));
	else
		switchEditViewTo(MAIN_VIEW);
...
bool Notepad_plus::fileLoadSession(const TCHAR *fn)	{
	...
	if (!fn)	{
		...
		FileDialog fDlg(_pPublicInterface->getHSelf(), _pPublicInterface->getHinst());
		const TCHAR *ext = nppGUI._definedSessionExt.c_str();
		generic_string sessionExt = L"";
		if (*ext != '\0')	{

			if (*ext != '.')
				sessionExt += L".";
			sessionExt += ext;
			fDlg.setExtFilter(L"Session file", sessionExt.c_str(), NULL);
		}
		fDlg.setExtFilter(L"All types", L".*", NULL);
	...
	if (sessionFileName)	{
		...
		if (!isEmptyNpp && (nGUI._multiInstSetting == multiInstOnSession || nGUI._multiInstSetting == multiInst))	{

			TCHAR nppFullPath[MAX_PATH];
			::GetModuleFileName(NULL, nppFullPath, MAX_PATH);


			generic_string args = L"-multiInst -nosession -openSession ";
			args += L"\"";
			args += sessionFileName;
			args += L"\"";
			::ShellExecute(_pPublicInterface->getHSelf(), L"open", nppFullPath, args.c_str(), L".", SW_SHOW);
			result = true;
		}
		else	{
			...
			if (!isAllSuccessful)
				param.writeSession(session2Load, sessionFileName);
		}
		if (result == false)	{
...
const TCHAR * Notepad_plus::fileSaveSession(size_t nbFile, TCHAR ** fileNames, const TCHAR *sessionFile2save)	{
	...
	if (sessionFile2save)	{
		...
		param.writeSession(currentSession, sessionFile2save);
